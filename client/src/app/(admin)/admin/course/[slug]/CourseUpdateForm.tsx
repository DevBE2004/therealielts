...
export default function UpdateCourseForm({
  ...
  useEffect(() => {
    const loadInitial = async () => {
      ...
      if (initialData?.benefit && Array.isArray(initialData.benefit)) {
        setBenefits(initialData.benefit as Benefit[]);
      } else {
        setBenefits([]);
      }
      ...
      reset({
        ...
        level: initialData.level || [0, 0],
        target: initialData.target || 0,
        routeId: initialData.routeId || "",
        categoryId: initialData.categoryId,
        benefit: initialData.benefit || [],
        slug: initialData.slug,
        urlYoutube: initialData.urlYoutube,
        descriptionSidebar: initialData.descriptionSidebar,
      ...
      if (
        ...
        initialData.images.every((img) => typeof img === "string")
      ) {
        const urls = initialData.images as string[];

        // ogImage
        setOgImagePreview(urls[1]);
        // phần preview ảnh chính
        const mainPreviews = urls.filter((_, idx) => idx !== 1);
        setPreviews(mainPreviews);

        // convert sang File[]
        const files = await Promise.all(
          urls.map((imgUrl, idx) => urlToFile(imgUrl, `image-${idx}.png`))
        );

        // console.log("FILES FILES: ", files)
        // bỏ ogImage khỏi mảng images chính
  ...
  // console.log("PREVIEWS COURSE: ", previews)
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
      ...
      <div className="space-y-2 md:col-span-2">
        <label className="text-sm font-medium">
          Mô tả Sidebar (Xuống dòng cho mỗi mô tả){" "}

...
type PackageCreateRequest = z.infer<typeof PackageCreateRequestSchema>;

type Props = {
  categoryId: number;
  onSubmitForm: (data: FormData) => void;
};
export default function CreatePackageForm({ categoryId, onSubmitForm }: Props) {
  ...
  const {
    ...
    watch,
    trigger,
  } = useForm<PackageCreateRequest>({
    resolver: zodResolver(PackageCreateRequestSchema),
    defaultValues: {
      level: [0, 0],
      benefit: [],
  ...
  const onSubmit = async (data: PackageCreateRequest) => {
